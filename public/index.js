// website can also be visited at cs353-ca3de.firebaseapp.com
...
function addStation(form){
    ...
    if(errors.length > 0){
        ...
        var newStation = db.collection("stations").add({
            ...
            position: new firebase.firestore.GeoPoint(Number(form.latitude.value),Number(form.longitude.value))
        }).then(value => {
            alert("Station added");
            location.reload()
        });
